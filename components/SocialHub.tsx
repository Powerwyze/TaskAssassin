...
interface SocialHubProps {
  ...
  onIssueTask: (toUserId: string, title: string, briefing: string) => void;
}
...
const SocialHub: React.FC<SocialHubProps> = ({
  ...
  const handleIssueTask = () => {
    if (!selectedUser || !taskTitle || !taskBriefing) return;
    onIssueTask(selectedUser.id, taskTitle, taskBriefing);
                    ...
                    return (
                        <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[80%] p-3 rounded font-mono text-sm ${
                                isMe ? 'bg-green-900/20 border border-green-500/50 text-green-100' : 'bg-slate-800 border border-slate-700 text-slate-300'
                            }`}>
                                {msg.text}
                            </div>
                        </div>
        ...
        <div className="pb-24 animate-in slide-in-from-right">
            ...
            <div className="space-y-4">
                ...
                <button 
                    ...
                    disabled={!taskTitle || !taskBriefing}
      ...
      <div className="flex border-b border-slate-700 mb-4">
        {(['ALLIES', 'WIRE', 'SEARCH'] as ListTab[]).map(tab => (
            <button 
                ...
                className={`flex-1 py-3 font-mono text-xs font-bold transition-colors relative ${
